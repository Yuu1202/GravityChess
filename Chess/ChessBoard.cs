    ...
    public class ChessBoard
    {
        ...
        private ChessBoard SetupBoard()
        {
            ...
            for (int i = 0; i < COLUMNS; i++)
            {
                ...
                boardArray[i, 0] = (ChessPiece)Activator.CreateInstance(
                                                Type.GetType("GravityChess." + playerPeices[i]));
                boardArray[i, 1] = (ChessPiece)Activator.CreateInstance(
                                                Type.GetType("GravityChess." + playerPeices[i + COLUMNS]));
                // Bidak pemain 1
                boardArray[i, ROWS - 1] = (ChessPiece)Activator.CreateInstance(
                                                Type.GetType("GravityChess." + playerPeices[i]), new object[] { 1 });
                boardArray[i, ROWS - 2] = (ChessPiece)Activator.CreateInstance(
                                                Type.GetType("GravityChess." + playerPeices[i + COLUMNS]), new object[] { 1 });
            }

            // Apply gravity to all pieces after setup
            ApplyGravityToAllPieces();

            return this;
        }

        // Terapkan gravitasi ke semua bidak: semua bidak langsung jatuh ke kolom paling kanan di baris masing-masing
        private void ApplyGravityToAllPieces()
        {
            // Untuk setiap baris, geser semua bidak ke kanan sejauh mungkin
            for (int row = 0; row < ROWS; row++)
            {
                // Lakukan iterasi beberapa kali agar semua bidak bisa jatuh berantai
                for (int pass = 0; pass < COLUMNS; pass++)
                {
                    for (int col = COLUMNS - 1; col > 0; col--)
                    {
                        if (boardArray[col, row] == null && boardArray[col - 1, row] != null)
                        {
                            boardArray[col, row] = boardArray[col - 1, row];
                            boardArray[col - 1, row] = null;
                        }
                    }
                }
            }
        ...
        public bool ActionPiece(Point from, Point to, bool bypassValidaiton = false)
        {
            if (bypassValidaiton || PieceActions(from).Contains(to))
            {
                ...
                // Setelah bidak dipindahkan, terapkan gravitasi ke seluruh papan
                ApplyGravityToAllPieces();
